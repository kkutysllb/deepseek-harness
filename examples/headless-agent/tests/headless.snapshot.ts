...
describe('headless stream-json snapshots', () => {
  ...
  it('recovers from context overflow through an assembled compaction', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        QILIN_SNAPSHOT: 'replay',
        QILIN_SNAPSHOT_FILE: compactionSessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('replays the advanced toolchain through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        QILIN_SNAPSHOT: 'replay',
        QILIN_SNAPSHOT_FILE: advancedSessionFixture,
        QILIN_SNAPSHOT_CHILD_FILES: [
          join(advancedScenarioDir, 'session.1.jsonl'),
          join(advancedScenarioDir, 'session.2.jsonl'),
        ].join(delimiter),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
  ...
  it('replays two fresh Ralph rounds through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        QILIN_SNAPSHOT: 'replay',
        QILIN_SNAPSHOT_FILE: join(ralphScenarioDir, 'session.jsonl'),
        QILIN_SNAPSHOT_OVERRIDE: join(ralphScenarioDir, 'replay.override.json'),
        QILIN_SNAPSHOT_CHILD_FILES: [
  ...
  it('replays persistent PTY tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        QILIN_SNAPSHOT: 'replay',
        QILIN_SNAPSHOT_FILE: ptySessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
