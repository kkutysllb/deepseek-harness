...
import { OFFLOADED_IMAGE_TEXT } from '@qilin/llm'
...
// The qilin-acp-demo bin (the demo:acp entry), this example's cordis.yml, and
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown QILIN_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { QILIN_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    env: {
      ...
      QILIN_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // published-handle disposal failure.
  {
    ...
    env: { QILIN_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { QILIN_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.QILIN_SNAPSHOT),
  hasPwsh,
})
