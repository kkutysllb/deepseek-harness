...
""".strip()
    ...
    assert dumped_env["QILIN_CWD"] == str(tmp_path)
    assert dumped_env["QILIN_SESSION_ROOT"] == str(tmp_path / "sessions")
    assert dumped_env["QILIN_CORDIS_CONFIG"] == str(tmp_path / "cordis.yml")
...
json.dump({"QILIN_CORDIS_CONFIG": os.environ.get("QILIN_CORDIS_CONFIG")}, open(os.environ["ENV_DUMP"], "w"))
for line in sys.stdin:
    msg = json.loads(line)
    if msg.get("method") == "initialize":
...
def test_client_default_launch_uses_bundled_runtime_and_injects_default_config(
    ...
    if ambient_config is None:
        ...
        monkeypatch.setenv("QILIN_CORDIS_CONFIG", ambient_config)
    ...
    assert json.loads(env_dump.read_text())["QILIN_CORDIS_CONFIG"] == str(default_config)
...
def test_client_respects_explicit_config_over_bundled_default(
    ...
    with HarnessClient(
        HarnessConfig(env={"ENV_DUMP": str(env_dump), "QILIN_CORDIS_CONFIG": "./explicit.yml"})
    ) as client:
        client.initialize(provider="deepseek-official", cwd="/workspace", model="deepseek-v4-pro")

    assert json.loads(env_dump.read_text())["QILIN_CORDIS_CONFIG"] == "./explicit.yml"
