...
- id: sdk-jsonrpc-server
  name: '@qilin/sdk-jsonrpc-server'
- id: agent-core
  name: '@qilin/agent-spine-demo'
  config:
...
def _client(tmp_path: Path, launch_args: tuple[str, ...]) -> HarnessClient:
    return HarnessClient(
        HarnessConfig(
            ...
            cwd=str(tmp_path),
            env={
                "QILIN_CORDIS_CONFIG": "./cordis.yml",
                "QILIN_SESSION_ROOT": str(tmp_path / "sessions"),
                "QILIN_CWD": str(tmp_path),
...
def test_python_sdk_boots_minimal_jsonrpc_config(tmp_path: Path, mode: str) -> None:
    ...
    harness = DeepSeekHarness(
        ...
        env={
            "QILIN_MODEL": model,
            "QILIN_CONTEXT_WINDOW": "1000000",
            "QILIN_SYSTEM_PROMPT": "You are the Python SDK minimal boot test agent.",
        },
...
def test_zero_config_run_injects_bundled_default_cordis_config(
    ...
    monkeypatch.setenv("QILIN_RUNTIME_MODE", mode)
