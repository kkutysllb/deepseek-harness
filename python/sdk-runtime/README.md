...
The wheel installs a `qilin` console command and the `openkylin_runtime` Python module. `qilin` forwards its arguments to the bundled executable and requires a non-empty `QILIN_HOME`; it never falls back to `~/.qilin`.
...
- `main()` implements the installed `qilin` console command and rejects an absent or blank `QILIN_HOME` before replacing the Python process.
