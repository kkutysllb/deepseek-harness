...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@qilin/sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
the client SDK injects via ``$QILIN_CORDIS_CONFIG`` for zero-config runs — the
...
def bundled_default_config_path() -> Path:
    ...
    The client SDK injects this path via ``$QILIN_CORDIS_CONFIG`` when the caller
