...
describe('CI workflow', () => {
  ...
  it('keeps a required Wine Windows job, a non-blocking native Windows job with failover, and a master-only standby', () => {
    ...
    expect(windowsNative['runs-on']).toContain('QILIN_CI_FAILOVER_WINDOWS')
    expect(windowsNative['runs-on']).not.toContain('QILIN_CI_FAILOVER_LINUX')
    expect(windowsNative['runs-on']).toContain('self-hosted')
    expect(windowsNative['runs-on']).toContain('qilin-win-ci')
    expect(windowsNative['runs-on']).toContain('qilin-windows-2025-16core')
    expect(windowsNative.name).toBe('windows node 24 / native complete')
    expect(windowsNative.if).toBe("github.event_name == 'pull_request'")
    expect(windowsNative.env).toMatchObject({
      QILIN_COVERAGE_TEST_TIMEOUT_MS: '30000',
    })
