...
function workspaceFixture(options: {
  ...
  writeJson(join(shared, 'package.json'), { name: '@qilin/shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@qilin/split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@qilin/host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@qilin/client-consumer' })
