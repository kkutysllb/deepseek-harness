...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  'packages/accounts/account-core': { kind: 'none', reason: 'Durable account records are server-side state; no model request, prompt, or tool result carries them.' },
  'packages/accounts/account-auth': { kind: 'none', reason: 'Session verdicts, CSRF decisions, and valve resolution are server-side control flow; no model request, prompt, or tool result carries them.' },
  'packages/accounts/account-http': { kind: 'none', reason: 'Route dispatch, cookie serialization, and enforcement verdicts are server-side control flow; no model request, prompt, or tool result carries them.' },
  'packages/client/ui-accounts': { kind: 'none', reason: 'The settings surface renders Host HTTP responses in browser views; no model request, prompt, or tool result carries them.' },
  'packages/accounts/account-rbac': { kind: 'none', reason: 'Principal resolution, policy evaluation, and catalog filtering are server-side control flow; no model request, prompt, or tool result carries them.' },
  ...
  'packages/shell/shell-env': { kind: 'indirect', reason: 'The env service exposes managed OPENKYLIN_* facts through the shell tools (qilin-tool-bash/dsh-tool-pwsh); it registers no prompt or schema of its own.' },
