...
const QILIN_PREFIX = '@qilin/'
const CLIENT_WEB = '@qilin/client-web'
...
export interface ClientDeclaration {
  ...
  /** Whether the manifest declares a dynamic qilin.client row. */
...
function expectedSections(pkg: ClientPackage, staticInputs: ReadonlySet<string>): Map<string, ExpectedRule> {
  ...
  for (const name of pkg.inject) add(name, 'qilin.client.inject')
  return expected
}
...
function isInternalDsh(name: string): boolean {
  return name === CORDIS || name.startsWith(QILIN_PREFIX)
}
