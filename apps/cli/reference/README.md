...
Bundle names resolve from the qilin installation first, then from the profile directory. In-box bundles (`@qilin/base`, `@qilin/web-app`, `@qilin/headless`) therefore always come from the same installation as the running `qilin`; out-of-tree bundles come from the profile's pnpm-managed `node_modules`. A bare plugin `name` in any patch row resolves through the profile directory's Node parent-walk, which reaches the maintained installation fallback `$QILIN_HOME/profiles/node_modules` (one symlink per package the installation's app and bundles depend on, healed on every launch).

The `web` and `headless` profiles auto-initialize from shipped templates on first use (`web`: base + web-app; `headless`: base + headless). Any other missing profile fails loud with a hint to run `qilin plugin --profile <name> add <package>`.

### App arguments

The launcher's flags come first and end at the first token it does not recognize; everything from there on is handed to the booted profile verbatim through `ctx.cmdlineArgs`, where any injected app plugin may parse it ([`@qilin/cmdline`](../../../packages/boot/cmdline/README.md)). `qilin --profile web --port 8080` therefore reaches the web app's `--port`, `qilin --profile web --help` prints that app's help and boots nothing, and `qilin --help` (no profile to hand it to) prints the launcher's own. `-V`/`--version` prints the launcher's version when it appears before the app-argument boundary.
