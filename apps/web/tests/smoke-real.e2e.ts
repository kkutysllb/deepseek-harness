...
describe('qilin web keyless CLI smoke', () => {
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...
          QILIN_HOME: join(sessionsDir, '.dsh'),
  ...
  it('QILIN_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
