...
OpenKylin is an all-plugin Cordis agent harness forked from DeepSeek Harness. Read [docs/architecture.md](docs/architecture.md) before changing `packages/`; follow [docs/AGENTS.md](docs/AGENTS.md) for documentation.

## Pre-release stance: foundation over blast radius

**Remove at the first tagged release.** Until then, prefer correct foundations to shims: rename or repackage, updating every reference. Backends reject old on-disk formats. SQLite uses monotonic `SCHEMA_VERSION`; `qilin-session` keeps `SESSION_FORMAT_VERSION` at `0` with no compatibility promise.

**Application launch.** Only `openkylin` profiles launch supported Node apps; package bins, demos, and SDK argv escapes are forbidden ([rule](docs/architecture.md#application-launch)).
...
Run checks before pushes via [qilin-pre-push-checks](.agents/skills/dsh-pre-push-checks/SKILL.md); report only commands run. After `gh stack sync`, validate immediately; do not merge before checks pass.
...
- Npm packages are `@qilin/<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@deepseek-ai/cordis` is a peerDependency (+ dev) of every harness package.
...
- **Testing policy** — [docs/testing.md](docs/testing.md). Every non-trivial model- or product-user-visible change updates a keyless recorded-session snapshot; [snapshot ownership](snapshots/AGENTS.md) reserves the top-level tree for session-driven cases and keeps other expected outputs owner-local. Fixtures replay on macOS/Linux; fix fixtures, not normalizers.
