...
| --- | --- | --- | --- |
| `@qilin/storage` | `packages/storage/storage/` | `ctx.storage` (the hub) | ✓ |
| `@qilin/storage-json` | `packages/storage/storage-json/` | registers backend `json` | ✓ |
| `@qilin/storage-sqlite` | `packages/storage/storage-sqlite/` | registers backend `sqlite` | ✓ |
| `@qilin/storage-domain` | `packages/storage/storage-domain/` | mounts `ctx.storage.domain` | ✓ |
| `@qilin/workspace` | `packages/workspace/workspace/` | `ctx.workspaceRegistry` | ✓ |
...
(workspace lives in its own group rather than `packages/host/`: the host group's naming rule requires the `qilin-host-*` prefix while this package is named `qilin-workspace`; and the workspace entity is a domain concept, not bound to the host assembly tier. Unrelated to the existing `agent-instructions` package — that is an AGENTS.md instruction loader.)

Dependency direction: `qilin-workspace` → `qilin-domain` → `qilin-storage` ← the two backends. `qilin-workspace` additionally depends on the read-only face of `ctx.sessionPersistence` (attach's cwd check reads the session header; when the service is absent, attach rejects outright — no verification, no bookkeeping). The `ctx.sessions` running-check for session deletion moves into future work together with the cascade.

### `qilin-storage`: the storage hub

A pure registration hub, no IO of its own, no Config. The `Storage` service mounts at `ctx.storage` with two faces: `backend` (a `BackendRegistry`: `register(name, backend)` returns the disposer, duplicate names throw; `get(name)` throws `backend-not-found` for unknown names) and data-form mounting (`mount(form, facility)` over the merge-extensible `StorageForms` map, into which `qilin-domain` merges the `domain` key; unmounted access throws `form-not-mounted`). The signature text lives in `packages/storage/storage/src/index.ts` and `src/registry.ts`.

**Multiple backends stay mounted side by side**; which backend serves a domain is `qilin-domain`'s configuration (below), never a global either-or. Disposer semantics = remove the name from the table; closing the backend itself belongs to the backend package's effect closure, unregister first then close.
...
### `qilin-domain`: the domain data form
...
| --- | --- | --- |
| JSONL: temp write + fsync + link/unlink atomic publish, 0o700/0o600 permissions, Windows variant (win32.ts) | pure medium | copied by `qilin-storage-json` this phase (whole-file atomic rewrite is the same protocol); becomes the shared implementation at migration |
