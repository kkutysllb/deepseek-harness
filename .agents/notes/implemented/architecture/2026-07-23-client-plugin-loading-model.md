...
On top of that, client and host plugins register and load consistently: a package declares `qilin.client` once, the host scans the declaration into the boot graph, and the same Loader semantics govern entries on both sides.
...
Why is the roster yml rows and not a scan? Because which plugins compose into a deployment is a composition decision, not a package property — a package declaring `qilin.client` in the repo does not mean this deployment mounts it, so discovery-by-scan cannot make that call; the node half scans only what the tree actually mounted.
...
|---|---|
| Two-axis taxonomy (entry × arrival) with infrastructure packages lacking `qilin.client` | Erased manifest dependency edges (inject leaked to the composer), split the plugin shape in two, blinded the purity gate to half the plugins |
