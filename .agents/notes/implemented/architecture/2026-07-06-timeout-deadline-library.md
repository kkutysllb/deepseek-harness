...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `qilin-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `qilin-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `qilin-timeout` (`idleWatchdog`) |
| Clear the timer | `qilin-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `qilin-timeout` (`timeoutOf`) |
