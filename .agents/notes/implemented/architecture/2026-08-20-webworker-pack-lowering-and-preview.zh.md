...
两个包以 `@qilin/experimental-*` 名义放在 `packages/experimental/`，私有且在官方发布之外。承载产品承诺的边界仍在产品包里：注入表、`__QILIN_TRANSPORT__` 与 `/plugins` bundle 字节由 `qilin-host-webserver`、`qilin-client-modules`、`qilin-client-connection` 拥有。
