...
The central `qilin-invariants` service owns only configuration, registration uniqueness, child-fiber lifecycle, rollback, disposal, and package-attributed failure. It exposes no generic plugin-shape, service-shape, or startup-assertion helpers and imports no product package.
...
|---|---|
| `qilin-session` | Strict sequence growth, turn/step enclosure, and same-step tool call/result pairing. |
| `qilin-agent` | Non-repeating agent status and terminal disposal transitions. |
| `qilin-scope` | Scoped-event carrier presence and routed-subject consistency. |
| `qilin-agent-loop` | Explicitly marked, frozen loop request reconstruction from the session event log. |
| `qilin-llm` | Stream block grammar, delta type/index matching, single usage, closed blocks, and terminal finish. |
| `qilin-llm-retry` | Durable retry records identify the open turn's latest closed step, remain unique per step, increase monotonically, and stay within retry and non-negative timer bounds. |
| `qilin-tools` | Monotonic pre/execute/post stages and immutable final execution/result snapshots. |
| `qilin-system-prompt` | Authoritative assembly section, tool, and variable data constraints. |
| `qilin-compaction` | Compaction start/summary/end pairing, range endpoints, token counts, and successful-summary presence. |
| `qilin-hook-protocol` | Hook invocation/result correlation, dialect, identity, and duration constraints. |
| `qilin-sandbox-policy` | Durable `sandbox/mode` events use the closed sandbox-mode vocabulary. |
| `qilin-fs` | Filesystem decision/observation events carry usable target and version identities. |
| `qilin-goal` | Durable goal snapshots preserve source attribution, rendered content, revisions, lifecycle and timestamp relationships, and sequential admitted rounds. |
| `qilin-goal-round-driver` | Goal-sourced continuation messages match the prompt reconstructed from the preceding durable goal state. |
| `qilin-subagent` | Provider add/remove and child start/end events preserve identity and pairing. |
| `qilin-permission-presets` | Durable permission decisions name a preset in the active permission table. |
| `qilin-user-approval` | Approval asked/decided records pair by call and use valid outcomes and policies. |
| `qilin-workflow` | Workflow and child-agent start/end events preserve run metadata, identity, outcome, count, and error relations. |
| `qilin-jobs` | Current and terminal task snapshots preserve id/kind, owner, status, and timestamp relationships. |
| `qilin-tool-todo` | Durable whole-list snapshots use unique trimmed items and closed statuses. |
| `qilin-time-context` | Plugin-attributed clock readings agree with the session's open turn, next pre-step position, and elapsed baseline; rendered time parses and does not postdate its event. |
