# Agent Note: Standalone sdk-minimal profile under openkylin
...
A minimal SDK agent needs an explicit plugin roster. Expressing it as an overlay on the full `sdk` profile leaves every `qilin-base` service mounted and makes exclusion depend on filters and disable entries spread across unrelated plugins. A later base row can change runtime behavior even when the model-facing tools remain filtered.

A complete caller-supplied Cordis tree gives an exact roster but bypasses profile initialization, bundle resolution, persistent plugin management, home and invocation patch layers, and the `openkylin`-owned process lifecycle. The minimal mode needs composition-level exclusion without creating another launcher or Python-owned application.

## Decision

### Launch and ownership

`openkylin --profile sdk-minimal` is a shipped startup-only profile. Its manifest lists only `@qilin/sdk-minimal`; it does not list `@qilin/base`. The bundle inserts the complete Cordis tree over the launcher's empty profile root, while the profile patch, home patch, and ordered invocation patches retain their ordinary precedence above it.

The `openkylin` CLI remains the only application launcher. The Python example selects `sdk-minimal` through the public `profile` field and an explicit Harness home. Python exposes no complete-config or arbitrary-argv path. The full Python and TypeScript SDK defaults remain `sdk`.

The bundle reuses `@qilin/sdk-app` for command help, stdin EOF, and bounded shutdown. The startup provider accepts a profile-name config so both SDK profiles render their actual command without duplicating process lifecycle code.

### Explicit composition

The bundle owns one DeepSeek adapter, SDK JSON-RPC serving, the explicit agent core, local subprocess and unrestricted filesystem providers, a platform-selected persistent shell, the string-replace editor, and uncompressed JSONL sessions under `$OPENKYLIN_HOME/sessions`. Linux and macOS mount Bash; Windows mounts PowerShell. The SDK initialization request owns the model id; `OPENKYLIN_CONTEXT_WINDOW` supplies fallback capacity for models outside the adapter's advisory catalog. The persona comes from `OPENKYLIN_SYSTEM_PROMPT`, and the credential from `DEEPSEEK_API_KEY`.
...
**Remove Web packages from the Python runtime closure.** Rejected because the wheel distributes the ordinary `openkylin` application and Python deployments may also need `openkylin web`; profile selection, not packaging divergence, separates those applications.
