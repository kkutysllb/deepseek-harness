# Agent Note: Standalone sdk-minimal profile under qilin
...
A complete caller-supplied Cordis tree gives an exact roster but bypasses profile initialization, bundle resolution, persistent plugin management, home and invocation patch layers, and the `qilin`-owned process lifecycle. The minimal mode needs composition-level exclusion without creating another launcher or Python-owned application.

## Decision

### Launch and ownership

`qilin --profile sdk-minimal` is a shipped startup-only profile. Its manifest lists only `@qilin/sdk-minimal`; it does not list `@qilin/base`. The bundle inserts the complete Cordis tree over the launcher's empty profile root, while the profile patch, home patch, and ordered invocation patches retain their ordinary precedence above it.

The `qilin` CLI remains the only application launcher. The Python example selects `sdk-minimal` through the public `profile` field and an explicit Harness home. Python exposes no complete-config or arbitrary-argv path. The full Python and TypeScript SDK defaults remain `sdk`.
...
The bundle owns one DeepSeek adapter, SDK JSON-RPC serving, the explicit agent core, local subprocess and unrestricted filesystem providers, a platform-selected persistent shell, the string-replace editor, and uncompressed JSONL sessions under `$QILIN_HOME/sessions`. Linux and macOS mount Bash; Windows mounts PowerShell. The SDK initialization request owns the model id; `QILIN_CONTEXT_WINDOW` supplies fallback capacity for models outside the adapter's advisory catalog. The persona comes from `QILIN_SYSTEM_PROMPT`, and the credential from `DEEPSEEK_API_KEY`.
...
**Remove Web packages from the Python runtime closure.** Rejected because the wheel distributes the ordinary `qilin` application and Python deployments may also need `qilin web`; profile selection, not packaging divergence, separates those applications.
