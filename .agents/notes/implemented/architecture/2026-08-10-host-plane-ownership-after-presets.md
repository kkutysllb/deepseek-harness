...
Three limits stay open and are recorded where they bite rather than fixed here: projection key presence is not a per-session capability signal ([`qilin-session-projection`](../../../../packages/session/session-projection/README.md)); a superseded standing generation is never reclaimed, which the settings-page authoring flow turns into a per-save cost ([`qilin-agent-presets`](../../../../packages/preset/agent-presets/README.md)); and a temporary plugin mounted through `cordis_mount` belongs to the composition rather than the session that mounted it ([`qilin-tool-cordis`](../../../../packages/extensions/tool-cordis/README.md)).
