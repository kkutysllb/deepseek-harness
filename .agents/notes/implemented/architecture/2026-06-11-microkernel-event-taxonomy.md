...
The event vocabulary lives in contract packages (`qilin-agent` declares the `agent/*` events); `@qilin/agent-loop` is the only concrete loop plugin and is itself swappable — nothing outside it may depend on it.
