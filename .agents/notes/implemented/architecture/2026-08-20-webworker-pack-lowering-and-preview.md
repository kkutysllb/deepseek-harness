...
Both packages live in `packages/experimental/` as `@qilin/experimental-*`, private and outside official releases. The boundary that carries product promises stays in the product packages: the injection table, `__QILIN_TRANSPORT__`, and the `/plugins` bundle bytes are owned by `qilin-host-webserver`, `qilin-client-modules`, and `qilin-client-connection`.
