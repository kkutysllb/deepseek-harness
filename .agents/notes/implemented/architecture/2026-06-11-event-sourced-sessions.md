...
- Persistence stays a plugin concern; the in-memory store ships in qilin-session.
...
- Derivation cost grows with log length — compaction (qilin-compaction) is the intended mitigation, not log mutation.
