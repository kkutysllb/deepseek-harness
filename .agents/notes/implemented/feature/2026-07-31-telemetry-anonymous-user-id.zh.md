# Agent Note: 遥测匿名用户 id（$QILIN_HOME/.anonymous-user-id）与 OTel Resource 的 user.id
...
| 单独开关 | 无 | 任一消费方都可创建该身份；`QILIN_TELEMETRY_DISABLED` 会停止遥测上报，但不会禁用反馈确认或 DeepSeek 请求头 |
...
- 一个 `$QILIN_HOME` 在 OTel 回流中是一个稳定用户；不同 home 在构造上就是不同用户，无跨 home 关联机制。
