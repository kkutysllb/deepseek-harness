...
| Diagnostic type and byte limit | `qilin-subagent` | Consumers receive a bounded optional field separate from assistant output. |
| Foreground and Job presentation | `qilin-tool-subagent` and the generic Job runtime | Scheduling choice does not change the underlying failure fact. |
| Process cancellation and quiescence | Product Provider and `qilin-subprocess` | Result settlement still precedes idempotent whole-tree disposal. |
