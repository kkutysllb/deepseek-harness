...
**Keep `DISABLED` and instruct reporters to re-run with `QILIN_TELEMETRY_MODE=FEEDBACK_ONLY`.** Rejected: the session that exhibited the problem is the one worth uploading, and re-running loses it.
