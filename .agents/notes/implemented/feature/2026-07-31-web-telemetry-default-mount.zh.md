...
| 共享模式 | `QILIN_TELEMETRY_MODE`，默认 `DISABLED`；显式设置 `FULL` 或 `FEEDBACK_ONLY` 即启用 | 新 profile 不发出遥测网络请求，内部部署仍可使用两种上传策略 |
...
- 开发者运行没有遥测配置的 `dsh web` 时，不会发出遥测网络请求。内部部署需设置 `QILIN_TELEMETRY_MODE`，并可让 `QILIN_TELEMETRY_OTLP_URL` 指向其他 collector。
