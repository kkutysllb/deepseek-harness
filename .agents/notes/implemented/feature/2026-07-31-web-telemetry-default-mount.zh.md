# Agent Note: openkylin web 组合默认挂载会话遥测（OTel 上报）
...
| CI 隔离 | GitHub 工作流顶层 `env: OPENKYLIN_TELEMETRY_DISABLED: '1'` | 即使 CI 任务显式选择上传模式，纵深防御也会让测试会话留在本地 |
...
- 开发者运行没有遥测配置的 `openkylin web` 时，在记录 `/feedback` 之前不会发出遥测网络请求。内部部署需设置 `OPENKYLIN_TELEMETRY_MODE`，并可让 `OPENKYLIN_TELEMETRY_OTLP_URL` 指向其他 collector。
