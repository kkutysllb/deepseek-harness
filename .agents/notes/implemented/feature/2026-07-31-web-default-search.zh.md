...
`packages/bundle/base/cordis.patch.yml` 明确挂载 `qilin-web`，配置 `searchProvider: deepseek-official` 与 `fetchProvider: http`，同时挂载 `qilin-web-search-deepseek`、`qilin-web-fetch-http`，并以 `searchTimeoutMs: 60000` 挂载 `qilin-tool-web`。[共享 base 的 Web 抓取默认值](2026-09-01-shared-base-web-fetch-default.zh.md)负责当前的 `fetch: true`；本文继续负责提供方选择、搜索凭据与超时。显式提供方 id 使选择不受注册顺序影响，同时个人覆盖层或 `--patch` 覆盖层仍可替换或禁用这些配置项。已交付的一分钟预算用于覆盖一次辅助 DeepSeek Messages 请求及服务端检索，同时保持 `qilin-tool-web` 提供方无关的 30 秒默认值不变，以供自定义组合使用。[Web 能力 seam 决策](../architecture/2026-06-24-web-capability-seam.zh.md)负责公开抓取安全策略。
