...
- `parseExitStatus` becomes public contract surface on `@qilin/shell`; `dsh-tool-bash/src/render.ts` keeps re-exporting it, so no bash-tool consumer changes.
