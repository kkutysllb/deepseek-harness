...
- commands 包新增对 `qilin-attachment` 与 `qilin-llm` 的依赖，`commands/execute` 携带必填的 `images` wire 参数——每个调用方都显式陈述其信封。
