# Agent Note: Default session-telemetry mount (OTel reporting) in the qilin web composition
...
| Sharing mode | `QILIN_TELEMETRY_MODE`, default `FEEDBACK_ONLY` ([feedback-gated default](2026-08-25-feedback-gated-telemetry-default.md)); explicit `FULL` or `DISABLED` overrides | A fresh profile uploads only when the user records `/feedback`, while internal deployments retain both explicit policies |
| Endpoint | `QILIN_TELEMETRY_OTLP_URL`, default `https://harness-telemetry.deepseeksvc.com/v1/logs` | Internal collector; the env override serves local/dev runs |
| Hard opt-out | any non-empty `QILIN_TELEMETRY_DISABLED` (including `0`/`false`) disables the row | The launcher patch takes effect before load-time transport validation and overrides every configured mode |
...
| CI isolation | top-level `env: QILIN_TELEMETRY_DISABLED: '1'` in GitHub workflows | Defense in depth keeps test sessions local even when a job explicitly selects an uploading mode |
...
- A developer running `qilin web` without telemetry configuration makes no telemetry network request until they record `/feedback`. An internal deployment sets `QILIN_TELEMETRY_MODE` and may point `QILIN_TELEMETRY_OTLP_URL` at another collector.
