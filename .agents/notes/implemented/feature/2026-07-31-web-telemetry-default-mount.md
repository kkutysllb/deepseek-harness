...
| Sharing mode | `QILIN_TELEMETRY_MODE`, default `DISABLED`; explicit `FULL` or `FEEDBACK_ONLY` opts in | A fresh profile makes no telemetry network request, while internal deployments retain both upload policies |
...
- A developer running `dsh web` without telemetry configuration makes no telemetry network request. An internal deployment sets `QILIN_TELEMETRY_MODE` and may point `QILIN_TELEMETRY_OTLP_URL` at another collector.
