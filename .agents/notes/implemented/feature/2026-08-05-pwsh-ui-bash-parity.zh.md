...
- `parseExitStatus` 成为 `@qilin/shell` 公开约定的一部分；`dsh-tool-bash/src/render.ts` 继续再导出它，bash 工具消费方零改动。
