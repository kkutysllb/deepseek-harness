...
The local provider scans cwd-sensitive project roots, custom roots, and user roots in first-wins rank order: project `.qilin`, project `.agents`, `customSkillDirs`, user `.qilin`, then user `.agents`. The user `.qilin/skills` scan skips `.system` so a system-owned directory is not treated as normal user content. The local provider does not synthesize built-in system skills; configured bundled roots and dedicated providers supply additional skills.
...
**Materialize built-in DSH authoring skills under `~/.qilin/skills/.system`.** Rejected because bundled skills do not write user home on startup, and embedded or remote providers supply configured skills.
