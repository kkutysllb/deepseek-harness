...
启动交付 profile（`qilin web`、`qilin --profile headless`、一次性任务）的 Windows 主机默认获得 PowerShell 栈；POSIX 主机不变。
...
- **冷启动的模块解析已恢复。** profiles 重构把 pwsh 包从 `apps/cli` 的依赖闭包中删掉了，`healProfilesModuleFallback` 因此从未把它们链接进 `$QILIN_HOME/profiles/node_modules`，新 Windows 主机解析不到 pwsh 行。`apps/cli` 与 `qilin-base` 声明 `qilin-pwsh-sandbox`/`qilin-tool-pwsh`，执行器的依赖链提供 `qilin-pwsh-local`；按仓库惯例，base bundle 把每个行插件都列为依赖。
...
- Keyless：`qilin --profile <name> --dump-config` 在同一份共享 patch 层中显示两个栈，每个行以自己的 `disabled` 表达式在挂载时决定清单。
