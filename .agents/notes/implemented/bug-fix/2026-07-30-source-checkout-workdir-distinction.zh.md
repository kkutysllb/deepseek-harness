...
直接断言 checkout 不是工作目录同样不准确。`qilin meta` 会有意让源码 checkout 同时充当这两个值。
...
路径推导方式与全局 `harness:source` 所有权保持不变。该段使用 first-party 顺序 −900，紧随 `harness:identity`。将两者描述为概念上独立、而不是始终不相等，使这条指令在普通项目会话和 `qilin meta` 中都准确。
...
**声明 checkout 永远不是工作目录。**拒绝：`qilin meta` 会有意让它们指向同一路径。
