...
Appearance, Language, and busy-Enter choices follow the DSH user home across reloads, ports, and loopback origins. Direct edits to `settings.yaml` converge through the existing invalidation stream, while legacy `openkylin.theme`, `openkylin.locale`, and `openkylin.conversation.busyEnter` entries are neither read nor written.
