...
上传脚本只接受 3 个 README 图片文件名，验证每个源文件均为 PNG，并以 `Content-Type: image/png` 和 `Cache-Control: no-store` 上传到 `dp-cdn-deepseek/harness/readme/`。脚本检查 OBS 响应状态、报告对应公开 URL，并在成功或失败后关闭客户端。仓库级 GitHub Actions Secret 提供 `OBS_OPENKYLIN_README_ACCESS_KEY_ID` 和 `OBS_OPENKYLIN_README_SECRET_ACCESS_KEY`；OBS 身份只需拥有该对象前缀的写权限。
