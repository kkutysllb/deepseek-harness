...
`packages/*/tool-*` 下已发布的产品工具包，每个都使用默认配置启动，包括 `qilin-tool-bash`（`bash`）、`qilin-tool-jobs`（`job_output`、`job_list`、`job_kill`）和 `qilin-tool-subagent`（`subagent`）。仅供示例使用的工具不在范围内。
