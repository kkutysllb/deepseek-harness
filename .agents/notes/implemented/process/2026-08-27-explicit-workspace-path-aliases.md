...
The generator emits an alias only for a package whose declared name is exactly `@qilin/<directory>`, because that is the only shape a wildcard could ever have resolved: it substituted the specifier's suffix into `packages/<group>/<suffix>/src`. Packages named after something other than their directory — `@qilin/typert-protocol` at `packages/typert/protocol`, the `qilin-client-*` and `qilin-host-*` families — already carry hand-written aliases and are left alone. A specifier claimed by two package directories throws rather than picking one, because an explicit map cannot express the group-order tiebreak the wildcard used; no such collision exists today.
...
Four wildcards remain, each with a single candidate: `qilin-host-*/invariant`, `qilin-client-*/invariant`, `qilin-client-*/client`, and the five `qilin-host-<name>/*` subpath maps. One candidate costs one probe, so expanding them would trade file size for nothing.

## Resolution differences this change makes

Every `@qilin/*` specifier appearing in repository sources — 1,023 distinct — resolves to the same target as before, with eleven exceptions that now resolve where they previously did not. All eleven previously reached built `lib/` output through the workspace symlink rather than source.

Seven are `/invariant` subpaths: `qilin-invariants/invariant`, `qilin-lsp/invariant`, `qilin-lsp-stdio/invariant`, `qilin-tool-lsp/invariant`, `qilin-terminal/invariant`, `qilin-terminal-bash/invariant`, and `qilin-tool-terminal/invariant`.
...
The other four are whole packages the coverage assertion surfaced: `qilin-client-ui-directory-picker-browse`, `qilin-client-ui-directory-picker-native`, `qilin-experimental-agent-team-profile`, and `qilin-experimental-agent-team-web-profile`. Each is named `dsh-<group>-<directory>`, which no wildcard could ever substitute, and each sits beside siblings that do carry hand-written aliases — they were simply missing. They now carry one too.
...
The win is confined to the tsx source launch. Vitest resolves through `vite-tsconfig-paths`, which matches in-process and checks file existence without ever constructing a Node module error, so it never paid the decoration cost: an A/B over one package's suite measured 5,934/5,829/5,908 ms against 5,878/5,851/5,905 ms, which is noise. Repository gate scripts import few `@qilin/*` packages and likewise show no separable difference. Shipped users run built `lib/` under plain Node and were never affected.
