...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `qilin-tool-bash` (`bash`), `qilin-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `qilin-tool-subagent` (`subagent`). Example-only tools are excluded.
