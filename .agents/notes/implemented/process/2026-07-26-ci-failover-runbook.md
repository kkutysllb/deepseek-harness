...
1. Repository **Settings → Secrets and variables → Actions → Variables → New repository variable**: name `OPENKYLIN_CI_FAILOVER_LINUX` (Linux pool outage) or `OPENKYLIN_CI_FAILOVER_WINDOWS` (Windows pool outage), value `selfhosted`.
...
Delete the `OPENKYLIN_CI_FAILOVER_LINUX` or `OPENKYLIN_CI_FAILOVER_WINDOWS` variable (or set it to anything other than `selfhosted`). New runs resolve back to the hosted enterprise pools. Remove any extra instances that were registered during the incident.
