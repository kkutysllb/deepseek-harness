...
Each declares itself in its own `package.json` under a `dsh` field: `qilin.profile` lists a profile's bundles, and `qilin.bundle` points at a bundle's patch file.

[`qilin-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`qilin-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`qilin-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
