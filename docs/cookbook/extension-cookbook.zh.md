...
| subagent 委派 | `ctx.subagents` 提供方注册表（`qilin-subagent-spawn-in-process`/`qilin-subagent-fork-in-process`/`qilin-subagent-acp`/`qilin-subagent-codex`/`qilin-subagent-claude-code`/`qilin-subagent-dsh-sdk`）+ `qilin-tool-subagent` 向模型暴露一个已配置的提供方 |
