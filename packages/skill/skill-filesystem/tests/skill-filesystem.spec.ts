...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.openkylin/skills'), 'same', 'user openkylin skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.openkylin/skills'), 'same', 'project openkylin skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.openkylin/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project openkylin skill')
    ...
    await writeSkill(join(noGit, '.openkylin/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.openkylin/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.openkylin/skills'), 'runtime-name', 'User loses')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.openkylin/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.openkylin/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.openkylin/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.openkylin/skills/broken-link'))
    await symlink('/dev/null', join(home, '.openkylin/skills/device-link'))
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.openkylin/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.openkylin/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    ...
    const previousAgentsHome = process.env.OPENKYLIN_AGENTS_HOME
    const previousBundledSkillDir = process.env.OPENKYLIN_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.OPENKYLIN_HOME = join(envHome, '.openkylin')
      process.env.OPENKYLIN_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.OPENKYLIN_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.openkylin/skills'), 'env-skill', 'Env skill')
      ...
      process.env.OPENKYLIN_HOME = join(envHome, 'empty-dsh')
      delete process.env.OPENKYLIN_BUNDLED_SKILL_DIR
      process.env.OPENKYLIN_AGENTS_HOME = join(envHome, 'empty-agents')
      const empty = new Context()
      await empty.plugin(SkillRegistry)
      SkillFileSystem.apply(empty, { watch: false })
      expect(await empty.skills.list()).toEqual([])

      delete process.env.OPENKYLIN_AGENTS_HOME
      ...
      if (previousDshHome === undefined) {
        ...
        process.env.OPENKYLIN_HOME = previousDshHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.OPENKYLIN_AGENTS_HOME
      } else {
        process.env.OPENKYLIN_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.OPENKYLIN_BUNDLED_SKILL_DIR
      } else {
        process.env.OPENKYLIN_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
