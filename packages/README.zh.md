...
npm scope 为 `@qilin/*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@qilin/<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`accounts/`](accounts/README.zh.md) | 多用户账户：账户/会话域核心、会话/CSRF 鉴权服务与后续 HTTP/RBAC 面 | 产品：稳定 API |
...
| [`bundle/`](bundle/README.zh.md) | 可安装的 `qilin --profile` 补丁层 | 产品：稳定 API |
