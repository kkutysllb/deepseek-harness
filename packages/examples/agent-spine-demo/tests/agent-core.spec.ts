...
import { renderPrompt, TOOL_ORDER_REST } from '@qilin/system-prompt'
...
/**
 * Unit coverage for the @qilin/agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldDshHome = process.env.QILIN_HOME
  const oldAgentsHome = process.env.QILIN_AGENTS_HOME
  process.env.QILIN_HOME = await mkdtemp(join(tmpdir(), 'dsh-agent-spine-demo-home-'))
  process.env.QILIN_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'dsh-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldDshHome === undefined) {
      ...
      process.env.QILIN_HOME = oldDshHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.QILIN_AGENTS_HOME
    } else {
      process.env.QILIN_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@qilin/agent$'] },
      { package_blocklist: ['^@qilin/session$'] },
    ]) {
  ...
  it('shares top-level dshHome between local skills and the managed bash environment', async () => {
    ...
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ QILIN_HOME: home, QILIN_SHELL: '1' })
    await ctx.fiber.dispose()
  })
