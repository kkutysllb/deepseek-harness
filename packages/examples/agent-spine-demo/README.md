...
@qilin/llm              abstract LLM service + content-block vocabulary
@qilin/session          event-sourced session log + store
@qilin/session-title    log-backed title service + deterministic fallback
@qilin/system-prompt    prompt-section + tool-schema assembly
@qilin/tools            registry + guarded pre/around/post/final-result pipeline
@qilin/skill            skill provider registry
@qilin/skill-filesystem      local filesystem skill provider
@qilin/agent            agent registry + initiator scope + agent/* events
@qilin/goal             optional persisted same-session goal domain
@qilin/tool-goal        optional model-facing goal controls
@qilin/goal-round-driver     optional same-session goal-round driver
@qilin/llm-retry        provider-routed request retry policy
@qilin/jobs-local      generic background-job registry
@qilin/invariants       configurable invariant registry service
@qilin/session/invariant
@qilin/agent/invariant
@qilin/scope/invariant
@qilin/agent-loop/invariant
                                  package-owned relational checks
@qilin/tool-bash        the model-facing bash schema (unless toolBash=false)
@qilin/agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@qilin/tool-skill       session-prefix skill catalog + model-facing loader schema
@qilin/tool-jobs       job_output/job_list/job_kill schemas + completion notices
@qilin/agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@qilin/agent-spine-demo'
...
For example, `{ invariants: { enabled: true, package_allowlist: ['^@qilin/'], package_blocklist: ['agent-loop$'] } }` keeps the package-owned companions mounted but suppresses the blocked owner. Blocklist matches override allowlist matches; see [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md) for regex and lifecycle rules.
