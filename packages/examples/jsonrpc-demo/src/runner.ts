 ...
 *
 * @module @qilin/sdk-jsonrpc-demo/runner
 */

import { existsSync } from 'node:fs'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@qilin/app-boot'

/* v8 ignore start -- composition over tested app-boot/jsonrpc and executable acceptance paths */
const NAME = 'qilin-jsonrpc-agent'
...
export async function runJsonrpcAgent(bareModuleBaseUrl?: string): Promise<void> {
  ...
  const fromEnv = process.env['QILIN_CORDIS_CONFIG']
  ...
  if (configPath === undefined || !existsSync(configPath)) {
    process.stderr.write(
      `usage: ${NAME} <path/to/cordis.yml> (or set QILIN_CORDIS_CONFIG=<path>, which wins); the config is required — there is no built-in fallback\n`,
    )
    process.exit(1)
  }
