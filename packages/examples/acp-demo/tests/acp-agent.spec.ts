...
import { TOOL_ORDER_REST } from '@qilin/system-prompt'
import type { Message } from '@qilin/llm'
import { SessionId } from '@qilin/session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @qilin/acp-demo composition:
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  process.env.QILIN_HOME = join(home, '.dsh')
  process.env.QILIN_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
