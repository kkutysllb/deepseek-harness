...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07qilin> ')).toEqual({ text: 'qilin> ', prompt: true, promptTail: 'qilin> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('qilin> ')).toEqual({ text: 'qilin> ', prompt: false, promptTail: 'qilin> ' })
  })
