...
import { normalizeTerminalText, TerminalSanitizer } from '@qilin/terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07openkylin> ')).toEqual({ text: 'openkylin> ', prompt: true, promptTail: 'openkylin> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('openkylin> ')).toEqual({ text: 'openkylin> ', prompt: false, promptTail: 'openkylin> ' })
  })
