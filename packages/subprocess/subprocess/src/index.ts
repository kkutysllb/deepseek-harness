...
import { OPENKYLIN_ENV_PREFIX } from './types.ts'
 ...
 * `OPENKYLIN_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `OPENKYLIN_*` fact goes through the spec's explicit `env`,
 ...
 * would otherwise survive and read back as `$env:OPENKYLIN_*` in the child;
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(OPENKYLIN_ENV_PREFIX)) env[key] = value
  }
  return env
}
