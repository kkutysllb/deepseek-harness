...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@qilin/shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@qilin/hook-protocol'
import type { RunHookOptions } from '@qilin/hook-protocol'
 ...
 * executor (qilin-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what qilin-bash-local's resolve does for the fields runHook sets.
      return {
