...
| Package | Role |
|---|---|
| `@qilin/shell` (this) | Service Definition: abstract service + vocabulary types |
| `@qilin/bash-local` | Service Provider: local subprocesses |
| `@qilin/bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@qilin/tool-bash` | the model-facing tool schemas over `ctx.shell` |
