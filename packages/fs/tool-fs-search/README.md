...
await ctx.plugin(LocalSubprocessRuntime)                     // @qilin/subprocess-local
...
await ctx.plugin(LocalSpillStore)                           // @qilin/spill-local
```
...
| `timeoutMs` | `30000` | Cooperative tool-call budget attached to both tool definitions, enforced by `@qilin/tool-call-timeout-policy` through `exec.signal`; the subprocess seam's terminate escalation is the hard kill. |
