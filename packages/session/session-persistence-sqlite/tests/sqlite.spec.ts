...
import SessionPersistenceSqlite, {
  ...
  SCHEMA_VERSION,
} from '@qilin/session-persistence-sqlite'
import {
...
describe('SessionPersistenceSqlite physical packing', () => {
  it('loads from cordis.yml and packs through the assembled service', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@qilin/session'",
      "- name: '@qilin/session-persistence-sqlite'",
      '  config:',
    ...
    ctx.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@qilin/session') return SessionStore
        if (specifier === '@qilin/session-persistence-sqlite') {
