...
缓存是投影注册表检查点接口上的折叠捷径，存于 `per-record` 领域数据表中。它带来六项后果：读取绝不绕过领域写入链；每次后台写入都 fail-soft；`ver` 不匹配时丢弃而不迁移记录；记录必须通过实时单元的 `stateSchema`；写入通过无损 JSON 边界替换一份完整会话记录，并隔离违反纯 JSON 契约的单元状态（其行被丢弃并记警告，其余单元照常持久化）；日志领先，缓存跟随。
