...
The cache is a fold shortcut over the projection registry's checkpoint face, stored in a `per-record` domain data table. It commits to six consequences: reads never bypass the domain write chain; every background write is fail-soft; a `ver` mismatch discards rather than migrates a row; a record must pass the live unit's `stateSchema`; writes replace one complete session record through the lossless-JSON boundary, isolating a unit state that violates the plain-JSON contract (its row is dropped with a warning while the surviving units persist); and the log leads, the cache follows.
