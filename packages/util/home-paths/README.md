...
`resolveDshHome()` resolves the single-root DeepSeek Harness home. Precedence, highest first: an explicit configured path, `$QILIN_HOME`, then `~/.dsh`. The harness keeps all user data under one root.
...
`dshHomeDisplay()` names an active root symbolically for user-facing paths: `~/.dsh` for the default home, `$QILIN_HOME` for any configured home. It never leaks an absolute machine path.

`QILIN_HOME_DIR_NAME` owns the default user-data directory name: `.dsh`.
