...
Brand ids that cross package boundaries and could plausibly be confused — `ToolCallId` in `qilin-llm`, the shared agent/session `SessionId` in `qilin-session`, `JobId` in `qilin-jobs`, `LspProviderId` in `qilin-lsp`. Strings that never leave their owning package do not need this abstraction.
