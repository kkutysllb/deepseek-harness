 ...
 * flat fallback directory `$QILIN_HOME/profiles/node_modules` (one symlink per
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  ...
  headless: ['@qilin/base', '@qilin/headless'],
}
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`qilin: ${link} exists and is not a symlink; remove it so qilin can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$QILIN_HOME/profiles/node_modules`: one
 * symlink per package in the qilin app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`compaction`, `invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the qilin app's package.json.
 * @param home - the Harness home; defaults to {@link resolveDshHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveDshHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (subprocess,
    // compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const normalized: ProfileManifest = {
    ...
    qilin: {
      ...manifest.qilin,
      profile: { ...manifest.qilin?.profile, bundles: [...current] },
    },
  }
