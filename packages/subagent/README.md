...
The Codex and Claude Code packages are independent optional Profile Bundles. Install either or both with `qilin plugin --profile <name> add @qilin/subagent-codex @qilin/subagent-claude-code`, then restart that Profile; each package registers only its dormant Host provider. To grant a tool, copy a complete Agent Preset, remove `disabled` from each matching tool row, and start a new Session. Removing one package withdraws only that provider and its private runtime closure on the next Profile start.
