...
The standalone composition below shows the complete explicit capability. A Profile based on `@qilin/base` keeps its existing Job rows, adds the product provider and tool rows, and does not mount duplicate Job services.
