...
import { resolveExampleLaunch, runLoaderSmoke } from '@qilin/loader-smoke'
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      ...
      env: {
        QILIN_TEST_CHILD_COMMAND: childLaunch.command,
        QILIN_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        QILIN_TEST_CHILD_ENV: JSON.stringify({
