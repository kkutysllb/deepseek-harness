 ...
 * (`@qilin/subagent-acp`): publish after the child handshake,
...
import { DeepSeekHarness, type HarnessNotification } from '@qilin/sdk-client'
...
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `qilin-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `QILIN_CORDIS_CONFIG`). Merged after
