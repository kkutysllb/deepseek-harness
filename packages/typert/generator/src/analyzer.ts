...
function isDualFacePackage(manifest: Record<string, unknown>): boolean {
  const section = manifest.qilin
  const client = section !== null && typeof section === 'object'
    ? (section as Record<string, unknown>).client
    : undefined
  return client !== null
...
function isRemoteSegment(value: string): boolean {
  // Generation bootstraps workspace artifacts before the typert protocol package is built,
