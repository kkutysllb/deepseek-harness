---
description: "Server-side RBAC for the account system: per-request Principal resolution, the role permission baseline plus config-driven deny-wins resource policy, assembly-time catalog filtering, and the optional rbacAuth fence."
...
- [The request Principal (contract M)](#the-request-principal)
- [The role default matrix (contract L, B1 baseline)](#the-role-default-matrix)
- [The resource policy (contract L, B2)](#the-resource-policy)
- [Assembly-time catalog filtering (the system-prompt waterfall fence)](#assembly-time-catalog-filtering)
- [The principal carrier (server-side identity for agent scopes)](#the-principal-carrier)
- [The /api fence (the D3 enforcement point)](#the-api-fence)
- [Owner check (contract N)](#owner-check)
...
<a id="the-role-default-matrix"></a>
...
<a id="assembly-time-catalog-filtering"></a>
...
Judgment is a pure deny-wins evaluation over resolved principals; the waterfall listener, the `rbacAuth` service, and `bindRbacPrincipal` are thin mounts over that core, keeping the dependency arrow one-way — the package never imports the HTTP surface or the transport.
