---
description: "账号体系的服务端 RBAC:每请求唯一的 Principal 构造点、角色权限基线加配置驱动的 deny 恒胜资源策略、装配期目录过滤,以及可选的 rbacAuth 栅栏。"
...
- [请求 Principal(契约 M)](#the-request-principal)
- [角色默认矩阵(契约 L,B1 基线)](#the-role-default-matrix)
- [资源策略(契约 L,B2)](#the-resource-policy)
- [装配期目录过滤(system-prompt waterfall 栅栏)](#assembly-time-catalog-filtering)
- [Principal carrier(agent scope 的服务端身份)](#the-principal-carrier)
- [/api 栅栏(D3 强制点)](#the-api-fence)
- [属主检查(契约 N)](#owner-check)
- [模型体验(Model Experience)](#model-experience)
- [Known Limitations 与暂缓工作](#known-limitations-and-deferred-work)
...
判定是对已解析 Principal 的纯 deny 恒胜求值;waterfall listener、`rbacAuth` 服务与 `bindRbacPrincipal` 都是该内核之上的薄挂载,保持依赖箭头单向——本包从不 import HTTP 面与传输层。
