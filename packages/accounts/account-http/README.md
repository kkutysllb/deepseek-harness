---
description: "The account HTTP surface on the engine webServer: the /api/v1/auth route family, the /api authentication and CSRF gate, the legacy cookie security policy, the auth-endpoint Origin whitelist, and per-IP rate limiting on the credential endpoints."
...
This package is wiring, not session semantics: route handlers translate requests into `account-auth` service calls and answers into the legacy cookie and header formats, and the `apiAuth` gate exposes the same fence to the transport by name.
