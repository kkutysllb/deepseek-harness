---
description: "引擎 webServer 之上的账户 HTTP 面：/api/v1/auth 路由族、/api 鉴权与 CSRF 栅栏、旧版 cookie 安全策略、auth 端点 Origin 白名单与凭证端点的按 IP 限流。"
...
本包是接线而非会话语义：路由 handler 把请求翻译为 `account-auth` 服务调用、把应答翻译为旧版 cookie 与头格式，`apiAuth` 栅栏服务把同一道围栏按名暴露给传输层。
