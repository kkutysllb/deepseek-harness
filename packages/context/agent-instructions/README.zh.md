...
用户全局文件始终是 `$QILIN_HOME/AGENTS.md`，没有本地 overlay；两个候选列表只控制项目 scope。`$QILIN_HOME` 默认为 `~/.dsh`，已配置的 `~`、`~/...` 与 Windows 风格 `~\...` 前缀会基于操作系统 home 目录展开。非正数或非有限渲染预算会同时禁用基线与动态加载；已配置 `maxSourceBytes` 必须是正整数。
