...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@qilin/llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@qilin/session'
...
describe('workspace context request injection', () => {
  ...
  it('labels a custom dshHome as QILIN_HOME instead of pretending it is ~/.dsh', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
