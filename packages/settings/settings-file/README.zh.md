...
| `dshHome` | `$OPENKYLIN_HOME` 或 `~/.openkylin` | `path` 省略时使用的 harness home |
...
| [`src/invariant.ts`](src/invariant.ts) | 不变式伴生插件（无运行时不变式；文件往返、watcher 时序与原子写入行为由包测试证明，进程内提交关系归 `qilin-settings` 所有） |
