...
The host half of model-mounted dynamic packages: the definition registry, the `node:vm` sandbox and fiber lifecycle for host halves, the invoke handler table, and the run round trip a browser page carries out. Provided as `ctx.dynamicCordisRunner`. The model-facing tools live in [`@qilin/tool-cordis`](../tool-cordis/README.md); the browser half is loaded by [`@qilin/cordis-client-runner`](../cordis-client-runner/README.md).
