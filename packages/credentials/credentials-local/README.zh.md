...
|---|---|---|---|
| 继承的进程环境 | `env` | 否 | 始终优先 |
| `$QILIN_HOME/.credentials.yaml` 文档 | `file` | 是（`set`/`unset`） | 高于两个 `.env` 层 |
| `<invocation cwd>/.env` | `project-env` | 不在此处 | 高于用户 `.env` |
| `$QILIN_HOME/.env` | `user-env` | 不在此处 | 其余情况 |
...
| `dshHome` | `$QILIN_HOME` 或 `~/.dsh` | `path` 缺省时使用的 harness home。 |
| `watch` | `true` | 热发布外部编辑。 |
