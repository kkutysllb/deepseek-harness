...
| `$QILIN_HOME/.credentials.yaml` document | `file` | yes (`set`/`unset`) | over both `.env` layers |
| `<invocation cwd>/.env` | `project-env` | not here | over the user `.env` |
| `$QILIN_HOME/.env` | `user-env` | not here | otherwise |
...
| `dshHome` | `$QILIN_HOME` or `~/.dsh` | Harness home used when `path` is omitted. |
