...
npm scope: `@qilin/*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@qilin/<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`accounts/`](accounts/README.md) | Multi-user accounts: account/session domain core, session/CSRF auth services, and future HTTP/RBAC surfaces | Product — stable API |
...
| [`bundle/`](bundle/README.md) | Installable `qilin --profile` patch layers | Product — stable API |
