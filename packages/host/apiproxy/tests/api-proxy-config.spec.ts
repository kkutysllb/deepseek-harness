...
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@qilin/llm'
import { SettingsProvider, settingsNamespace } from '@qilin/settings'
...
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@qilin/agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('@qilin/settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('@qilin/credentials-local')
  })
