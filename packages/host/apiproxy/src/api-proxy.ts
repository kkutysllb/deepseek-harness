...
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@qilin/agent'
...
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@qilin/llm'
import { errorChain } from '@qilin/llm'
import type { ContentBlock, MessageSource } from '@qilin/llm'
import { isAppendSurfaceEvent, isJsonValue } from '@qilin/session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@qilin/session'
...
import { SubagentError } from '@qilin/subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@qilin/subagent'
...
import type { PresetBearingSession } from '@qilin/agent-presets'
...
import type { SessionRawArtifact } from '@qilin/session-persistence'
import {
...
import type { GoalRef as CoreGoalRef } from '@qilin/goal'
...
import { SettingsConflictError, settingsNamespace } from '@qilin/settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@qilin/settings'
...
import type { CallId } from '@qilin/llm/brand'
import type { ScopeKey } from '@qilin/scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@qilin/user-approval'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @qilin/session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @qilin/goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @qilin/settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @qilin/credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @qilin/session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @qilin/skill', details: {} })
        }
