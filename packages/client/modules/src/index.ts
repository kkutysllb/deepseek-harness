...
/** Resolved package metadata for one `qilin.client` package (cached per name, never expires). */
...
function parseQilinClient(pkgName: string, value: unknown): DshClientDeclaration | undefined {
...
const CLIENT_RUNTIME_ID = '@qilin/client-runtime'
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `qilin.client` declaration) are
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const section = pkg.qilin
    const decl = parseQilinClient(
      pkgName,
      section !== null && typeof section === 'object' ? (section as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares qilin.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
