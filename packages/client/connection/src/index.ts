...
import type { WebRoute, WebUpgradeRoute } from '@qilin/host-webserver'
import { toFetchHandler } from '@qilin/host-apiproxy'
  ...
  const route: WebRoute = {
    ...
    handler: async (req, res) => {
      ...
      if (rbac !== undefined) {
        ...
        const endpoint = pathname.startsWith(`${API_PATH}/`) ? pathname.slice(API_PATH.length + 1) : undefined
        if (endpoint !== undefined) {
          let verdict: RbacAuthVerdict
          try {
            verdict = rbac.checkRequest(req, endpoint)
          } catch {
            // A throwing gate is infrastructure damage seen at the fence:
            // map it to the stable 500, never to a permission refusal.
    ...
    const registerDownlink = (
      ...
      apiCtx.effect(() => apiCtx.webServer.registerUpgrade({
        ...
        handler: (req, socket, head) => {
          ...
          const gate: ApiAuthGate | undefined = apiCtx.get('apiAuth')
          if (gate !== undefined && !gate.checkUpgrade(req)) {
