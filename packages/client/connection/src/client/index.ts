...
import { FixtureApiClient, FixtureAuthClient } from './fixture.ts'
...
export interface ConnectionHandle {
  ...
  /**
   * Subscribe to session-loss (401) events from any carrier: business fetches
   * through the API client and account/auth calls share one signal, so the
   * UI subscribes once.
...
export function apply(ctx: Context): void {
  ...
  // One shared 401 signal for the whole handle: the API carrier taps it on
  // business fetches (WebApiClient only; a custom transport carrier has no
  // tap point here) and the account client emits its own auth failures into
  // the same channel, so consumers subscribe once.
  const unauthorized = new UnauthorizedSignal()
  const api: IApiClient = fixtureClient ?? transport?.createApiClient() ?? new WebApiClient({ unauthorized })
  const auth: IAuthClient = fixtureClient ? new FixtureAuthClient() : new AuthClient({ unauthorized })
